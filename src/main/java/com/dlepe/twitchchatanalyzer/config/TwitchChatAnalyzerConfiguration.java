...
import com.dlepe.twitchchatanalyzer.constant.TwitchChatAnalyzerConstants;
...
public class TwitchChatAnalyzerConfiguration {
	...
	public WebClient logsWebClient() {
		// Max buffer memory was increased to 64MB allow for downloading large logs
		final int size = 64 * 1024 * 1024;
		final ExchangeStrategies strategies = ExchangeStrategies.builder()
			.codecs(codecs -> codecs.defaultCodecs().maxInMemorySize(size)).build();

		return WebClient.builder().baseUrl(logApiBaseUrl).exchangeStrategies(strategies).build();
	}
	...
	WebClient twitchWebClient(
		...
		ExchangeFilterFunction oauthFilterFunction = buildOAuthExchangeFilter(
			registrationRepository, TwitchChatAnalyzerConstants.TWITCH_OAUTH_REGISTRATION_ID);
		return WebClient.builder().baseUrl(twitchHelixBaseUrl)
			.defaultHeader(TwitchChatAnalyzerConstants.CLIENT_ID,
				registrationRepository.findByRegistrationId(
					TwitchChatAnalyzerConstants.TWITCH_OAUTH_REGISTRATION_ID).block().getClientId())
	...
	ReactiveClientRegistrationRepository getRegistration(
		...
		ClientRegistration registration = ClientRegistration.withRegistrationId(
				TwitchChatAnalyzerConstants.TWITCH_OAUTH_REGISTRATION_ID).tokenUri(tokenUri)
