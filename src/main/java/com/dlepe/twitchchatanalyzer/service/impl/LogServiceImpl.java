...
public class LogServiceImpl implements LogService {
  ...
  public List<ChatLogRecord> getRawLogDataForDateRange(final String channelName,
    ...
    Flux.merge(startTime.toLocalDate()
        .datesUntil(endTime.toLocalDate()
            .plusDays(1))
        .map(day -> getRawLogData(channelName, day, startTime, endTime))
        .collect(Collectors.toList()))
  }

  private Mono<String> getRawLogData(final String channelName, final LocalDate logsDate,
      final LocalDateTime startTime, final LocalDateTime endTime) {
    log.info(String.format("Fetching log data for [channelName=%s] "
        + "[logDate=%s] [startTime=%s] [endTime=%s]", channelName, logsDate, startTime, endTime));
  ...
  private List<ChatLogRecord> readChatLogs(String chatLogs, final String channelName, final LocalDate logsDate,
      final LocalDateTime startTime, final LocalDateTime endTime) {

    // Optimize by grouping aggregating all chat text having similar timestamps
    // Also, consider skipping any chat text where chatText.length <
    // smallest monitored emote

    return Arrays.asList(Objects.requireNonNull(chatLogs)
            .split("\n"))
        .parallelStream()
        .map(logLine -> buildChatLogRecord(logLine, channelName))
        .filter((logRecord) -> Objects.nonNull(logRecord) && (logRecord.getLogTimestamp()
            .isAfter(startTime) || logRecord.getLogTimestamp()
            .isEqual(startTime)) && (logRecord.getLogTimestamp()
            .isBefore(endTime) || logRecord.getLogTimestamp()
            .isEqual(endTime)))
        .collect(Collectors.toList());
  ...
  private ChatLogRecord buildChatLogRecord(final String logLine, final String channelName) {
    try {
      ...
      final LocalDateTime logTimestamp = LogParseUtils.getTimestampToNearestMinute(logParts[0]);
  ...
  public Map<String, String> parseChatLogs(final VideoDetails videoDetails,
    ...
    final Map<String, String> bestMomentsPerEmote = new HashMap<>();
    final Map<String, Long> maxEmoteScore = new HashMap<>();
    ...
    chatLogs.forEach(logRecord -> {
      ...
      configuredEmotes.getKeywords()
          .keySet()
          .forEach(emotion -> {
            final long emoteScoreForTimestamp = timestamp.getChatMetrics()
                .getOrDefault(emotion, 0L);
            if (emoteScoreForTimestamp > maxEmoteScore.getOrDefault(emotion, 0L)) {
              maxEmoteScore.put(emotion, emoteScoreForTimestamp);
              bestMomentsPerEmote.put(emotion, timestamp.getTimestampUrl());
            }
          });
    });

    return bestMomentsPerEmote;
  }
  ...
  private VideoChatTimestamp createOrUpdateTimestamp(final LocalDateTime logTimestamp,
    ...
    final Optional<VideoChatTimestamp> existingRecord =
        videoChatTimestampRepository.findById(hashKey);
